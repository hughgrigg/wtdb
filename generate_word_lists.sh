...
if [ ! -f /tmp/dictionaries.list ]; then
  touch /tmp/dictionaries.list

  if [ -f /usr/share/dict/british-english ]; then
    cat /usr/share/dict/british-english | awk '$0 ~ /^[a-z]{2,10}$/' > /tmp/dictionaries.list
  fi

  if [ -f /usr/share/dict/american-english ]; then
    cat /usr/share/dict/american-english | awk '$0 ~ /^[a-z]{2,10}$/' >> /tmp/dictionaries.list
  fi
fi
